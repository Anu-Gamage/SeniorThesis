...
%    workspace
    ...
    if j == 1
        ...
        rand('twister',5489);
        [acc_val, nmi_val] = printResult(V{1}, label, options.K, options.kmeans);
    else
    ...
    %         [U{i}, V{i}] = NMF1(X{i}, K, options, U_, V{i-1});
        ...
        rand('twister',5489);
        [acc_val, nmi_val] =  printResult(V{i}, label, options.K, options.kmeans);
    end
end
...
while j < Rounds                            %Number of rounds of AO
    ...
    for i = 1:viewNum
        ...
        tmp1 = (X{i} - U{i}*V{i}');
        tmp2 = (V{i} - centroidV);
        logL = logL + sum(sum(tmp1.^2)) + alpha* (sum(sum(tmp2.^2)))+sum(sum((V{i}'*L).*V{i}'));  %????????SampleW??V'*L*V
    end
